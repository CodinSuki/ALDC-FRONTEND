...
export const fetchAdminDashboardData = async (): Promise<AdminDashboardData> => {
  const [
    ...
    supabase.from('project').select('projectid', { count: 'exact', head: true }),
    supabase.from('property').select('propertyid, propertyname, propertytypeid, propertylistingstatusid, is_archived, createdat'),
    supabase.from('propertytype').select('propertytypeid, propertytypename'),
    supabase.from('propertylistingstatus').select('propertylistingstatusid, propertylistingstatuscode, propertylistingstatusname'),
    supabase.from('transaction').select('*').order('transaction_date', { ascending: false }),
  ]);
  ...
  const transactionRows = transactionsRes.error
    ? []
    : ((transactionsRes.data ?? []) as Array<Record<string, unknown>>);
  ...
  try {
    ...
    if (inquiriesResponse.ok) {
      const payload = (await inquiriesResponse.json().catch(() => ({}))) as { items?: IntakeItem[] };
      const items = payload.items ?? [];
      activeInquiries = items.filter((item) => isActiveInquiryStatus(asText(item.status))).length;

      recentInquiries = items
        .slice()
        .sort((a, b) => new Date(asText(b.createdAt)).getTime() - new Date(asText(a.createdAt)).getTime())
        .slice(0, 5)
        .map((item) => ({
          client: asText(item.clientName, 'N/A'),
          property: asText(item.reference, 'N/A'),
